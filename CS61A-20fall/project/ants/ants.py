...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            # if place.ant.can_contain and not self.can_contain:
            #     place.ant.contain_ant = self
            # elif self.can_contain and not place.ant.can_contain:
            #     self.contain_ant = place.ant
            #     place.ant = self  // 这边的错误原因：函数的调用方式
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # while not currentplace.bees and not Place.is_hive(currentplace): 
        #     currentplace = currentplace.entrance
        # return bee_selector(currentplace.bees) if currentplace.bees else None
        # 1. ThrowerAnt和Place之间是组合关系，而不是继承关系。这意味着ThrowerAnt对象中包含一个Place对象的引用，而不是从Place继承。 其来自于Insect，在实例属性中有一个self.place，且Ant中并没有改变place属性的含义，那个就是对应的Place实例
        # 2. 不需要用is_hive来判断，因为beehive是一个特定的Place实例，代表蜂巢，这个实例在初始化游戏状态时被建立，并且在代码中被用来标识蜂巢的位置。（在gamestate类里）
        current_place = self.place
        distance = 0
        while current_place and current_place is not beehive:
            if current_place.bees and self.min_range <= distance <= self.max_range:
                return bee_selector(current_place.bees)
            current_place = current_place.entrance
            distance += 1 # 移动到下一个位置的时候同时增加距离
        return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    '''An wallant does nothing each turn but protects our glorious home base.'''
    
    name = 'Wall'
    food_cost = 4
    implemented = True # Use it in a game
...
# The HungryAnt Class
class HungryAnt(Ant):
    '''A hungryant selects a random bee from its place and eats it whole. After eating a bee, it must spend 3 turns chewing before eating again. If there is no bee available to eat, it will do nothing.'''
    
    name = 'Hungry'
    food_cost = 4
    chew_duration = 3 # a class attribute
    implemented = True
    ...
    def action(self, gamestate):
        if self.chewing > 0:
            ...
            if self.place.bees:
                ...
                bee.reduce_health(bee.health) # Eat the bee by reducing its health to 0
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and also deals 1 damage to all bees in its place each turn."""
    ...
    def action(self, gamestate):
        for bee in list(self.place.bees):
            bee.reduce_health(self.damage) # 注意前面的代码
...
# END Problem EC
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem EC
    true_queen_exists = False # 需要统一所有此类实例来进行管理，所以设为类属性更好
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    ...
    def __init__(self, health=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(health)
        if not QueenAnt.true_queen_exists: # 此类写法是针对所有的QueenAnt实例
    ...
    def action(self, gamestate):
        ...
        current_place = self.place.exit # 需要避免自身也被加强，所以从下一个exit开始
        while current_place:
            if current_place.ant: #  容器蚂蚁的包含蚂蚁不是实例，所以需要额外判断
                if isinstance(current_place.ant, ContainerAnt):
                    # Buff contained ant if present
                    if current_place.ant.contained_ant and not hasattr(current_place.ant.contained_ant, 'buffed'): # hasattr函数用于检查对象是否具有某个属性
                        current_place.ant.contained_ant.damage *= 2
                        current_place.ant.contained_ant.buffed = True
                # Buff the ant itself if not already buffed
                if not hasattr(current_place.ant, 'buffed'):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.health <= 0 and self.is_true_queen:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        def slow_status(bee, previous_action, gamestate):
            nonlocal length
            if length > 0:
        ...
        self.is_slowed = True
        self.apply_status(slow_status, self.action, length)
        # END Problem Optional 2
    ...
    def scare(self, length):
        ...
        if not self.has_been_scared:
            def scare_status(bee, previous_action, gamestate):
                nonlocal length
                if length > 0:
                    bee.is_scared = True
                    length -= 1
            ...
            self.is_scared = True
            self.apply_status(scare_status, self.action, length)
    ...
    def apply_status(self, status, previous_action, length):
        ...
        def new_action(gamestate):
            status(self, previous_action, gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    base_damage = 2
    distance_weakening = 0.25
    damage_reduction_per_insect = 0.0625
    implemented = True   # Change to True to view in the GUI
    ...
    def insects_in_front(self, beehive):
        ...
        insects = {} # 判断有哪些昆虫
        current_place = self.place
        distance = 0 #从当前位置开始判断，距离为0
        
        # 检查当前位置的蚂蚁和蜜蜂
        if current_place.ant is not None and current_place.ant is not self:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        damage = self.base_damage - distance * self.distance_weakening - self.insects_shot * self.damage_reduction_per_insect # 攻击
        return max(0, damage)
        # END Problem Optional 3
