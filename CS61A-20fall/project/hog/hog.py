...
def pig_pass(player_score, opponent_score):
    ...
    if player_score >= opponent_score:
        ...
        return opponent_score - player_score < 3
    # END PROBLEM 4b
    ...
    while max(score0, score1) < goal:
        if who == 0:
            ...
            if not extra_turn(score1, score0):
                who = other(who)
        say = say(score0, score1) # This line is PROBLEM 6 CODE
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        this_gains = this_score - last_score
        
        if this_gains > running_high:
            print(this_gains, "point(s)! The most yet for Player", who)
            return announce_highest(who, this_score, this_gains)
        else:
            return announce_highest(who, this_score, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        this_avg = make_averaged(roll_dice, trials_count)
        average_score = this_avg(i, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    dice_zero = free_bacon(opponent_score)
    after_zero = dice_zero + score
    if dice_zero >= cutoff:
        return 0
    elif extra_turn(after_zero, opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
