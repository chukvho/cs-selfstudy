...
class Player:
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        card = self.hand.pop(card_index) # 起初错误用成了remove，这个删除的是一个固定的值，同时返回None 
        return card
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        other_card.attack, other_card.defense = other_card.defense, other_card.attack # Python特有的交换式写法
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.deck.cards = [card for card in opponent.deck.cards if card.attack != other_card.attack and card.defense != other_card.defense]
